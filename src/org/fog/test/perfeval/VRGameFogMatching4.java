...
public class VRGameFogMatching4 {
...
//        }
		...
		try {
			...
			String appId_vs = "vs";
			String appId_vs1 = "vs1";

			
			FogBroker broker0 = new FogBroker("broker0");			
			...
			//Application application_vs1 = createApplicationVideoStreamAnalysis   (appId_vs1, broker0.getId());
			...
			//application_vs1.setUserId(broker0.getId());
			
			
			createFogDevices(broker0.getId(), appId_vs, appId_vs, appId_vs);
			//createFogDevices(broker0.getId(), appId_vs1, appId_vs1, appId_vs1);
...
//							:(new ModulePlacementEdgewards(fogDevices, sensors, actuators, application_vs, moduleMapping_0)));
	...
	private static void create_ds_sensor(FogDevice edge, int userId0, String app_ds, double delay) {
		...
		Sensor caSensor = new Sensor("s-"+app_ds, "CAMERA", userId0, app_ds,
				new DeterministicDistribution(100+(random(-362064))));//1000 
	...
	{
		//CPU_r = [400000* 10 ^ 6,79000* 10 ^ 6,76000* 10 ^ 6, 29000* 10 ^ 6,28000* 10 ^ 6,27000* 10 ^ 6,26000* 10 ^ 6,25000* 10 ^ 6,24000* 10 ^ 6,23000* 10 ^ 6,29000* 10 ^ 6,21000* 10 ^ 6,20000* 10 ^ 6,20000* 10 ^ 6]#MIPS
...
//		else if ( bw < 0)	bw = downBw;
	...
	private static Application createApplicationVideoStreamAnalysis(String appId, int userId)
	{
		...
		List<AppLoop> loops = new ArrayList<AppLoop>(){{add(loop3);add(loop2);add(loop1);}};
